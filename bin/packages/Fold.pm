...
@ISA = qw( Exporter );
@EXPORT = qw( getFoldingCD getFoldingHACA getIndependentFoldingHACA );
...
####################################################################################################
{
    ...
    ## VARIABLES
    ##
    my ( $seq, $cons, $start_box1, $start_box2 ) = @_;
    ...
    ##
    ## use RNAsubopt to produce all suboptimal foldings of each haripin within a given energy-range
    ##
...
####################################################################################################
{
    ...
    #`echo '$seq\n' '$cons' > cons`;   #write file needed by RNAfold
    $fold = `echo -e '$seq\n$cons' | $RNASUBOPT -C -e 1 -s`;   #fold with constraint
    @tmp = split(/\n/, $fold);
